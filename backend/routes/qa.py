...
async def create_qa_session(
    ...
    Args:
        ...
        generate_audio: Whether to generate TTS audio for the answer (default: True)
        db: Database session
        current_user: Authenticated user
        
    Returns:
        Created Q&A session with answer and audio
        
    Note:
        Uses FLAN-T5 (lightweight model) for answer generation with RAG
        Audio responses are generated by default
    """
    from backend.config import settings
    import os
...
async def ask_question_audio(
    ...
    """
    Ask a question via audio (STT + LLM processing + TTS response)
    ...
    Returns:
        Q&A session with transcribed question, text answer, and audio answer
    """
    # Verify lesson exists
    lesson = db.query(Lesson).filter(Lesson.id == lesson_id).first()
    if not lesson:
    ...
    if llm_service:
        try:
            ...
            # Generate TTS audio for answer (always generate for audio questions)
            if answer:
                try:
                    from stt_pipelines.uzbek_tts_pipeline import create_uzbek_tts
                    tts = create_uzbek_tts(voice="male_clear")
                    
                    audio_filename = f"qa_{lesson_id}_answer_{hash(answer[:50])}.mp3"
                    audio_path = os.path.join(settings.AUDIO_DIR, audio_filename)
